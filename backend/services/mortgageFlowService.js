...
function parseAmount(text) {
  ...
  if (/مليار|billion/.test(normalized)) {
    amount *= 1_000_000_000;
  } else if (/مليون|million/.test(normalized)) {
    amount *= 1_000_000;
  } else if (/ألف|الف|thousand/.test(normalized)) {
    amount *= 1_000;
  }
...
function extractAmountNearKeywords(message, keywords = []) {
  ...
  const escapedKeywords = keywords.map((keyword) => escapeKeyword(keyword)).join('|');
  const amountPattern =
    '(\\d+(?:[\\d,.]*\\d)?(?:\\s*(?:مليار|مليون|ألف|الف|billion|million|thousand))?)';

  const forwardMatch = message.match(
    new RegExp(`(?:${escapedKeywords})[^\\d]{0,20}${amountPattern}`, 'i')
  );
  const backwardMatch = message.match(
    new RegExp(`${amountPattern}[^\\d]{0,20}(?:${escapedKeywords})`, 'i')
  );
...
function extractAllAmounts(message = '') {
  return (
    message.match(
      /\d+(?:[\d,.]*\d)?(?:\s*(?:مليار|مليون|ألف|الف|billion|million|thousand))?/gi
    ) || []
  )
...
function extractName(userMessage = '', stage = '', existingData = {}) {
  ...
  if (stage !== 'collect_name' && !/^(?:انا|أنا|اسمي|إسمي|اسمى|الاسم)\b/i.test(normalized)) {
    return null;
  }
  ...
  if (
    /(أعزب|اعزب|متزوج|دخل|راتب|قسط|مقدم|شقة|وحدة|تمويل|عقار|موبايل|هاتف|رقم)/i.test(candidate)
  ) {
    return null;
  }
...
function extractMaritalStatus(userMessage = '') {
  ...
  if (/(متزوج|متجوز|متزوجة|married)/i.test(message)) {
    return 'married';
  }

  if (/(أعزب|اعزب|عزباء|single)/i.test(message)) {
    return 'single';
  }
...
function extractObligations(userMessage = '', stage = '') {
  ...
  if (
    stage === 'collect_obligations' &&
    /^(?:مفيش|مافيش|لا يوجد|بدون|ولا حاجة|صفر|none|no|0+)$/.test(compactValue)
  ) {
    return 0;
  }

  const amount = extractAmountNearKeywords(message, [
    'التزامات',
    'التزام',
    'قسط',
    'اقساط',
  ...
  if (stage === 'collect_obligations' && amounts.length === 1) {
    return amounts[0];
  }
...
function extractDownPayment(userMessage = '', stage = '') {
  ...
  if (
    stage === 'collect_down_payment' &&
    /^(?:مفيش|مافيش|معيش|بدون|لا يوجد|صفر|none|no|0+)$/.test(compactValue)
  ) {
    return 0;
  }
  ...
  if (stage === 'collect_down_payment' && amounts.length === 1) {
    return amounts[0];
  }
...
function extractData(userMessage, stage = '', existingData = {}) {
  ...
  let income = extractAmountNearKeywords(message, [
    'دخل',
    'راتب',
    'مرتب',
    'صافي',
  ...
  let propertyValue = extractAmountNearKeywords(message, [
    'سعر',
    'ثمن',
    'ميزانية',
    'الوحدة',
  ...
  if (!income && amounts.length === 1 && stage === 'collect_income') {
    income = amounts[0];
  }

  if (!propertyValue && amounts.length === 1 && stage === 'collect_property_price') {
    propertyValue = amounts[0];
  }
...
function calculateLoanFromInstallment(maxInstallment, annualRate, years) {
  ...
  if (!monthlyRate) {
    return Math.round(maxInstallment * totalMonths);
  }
...
function getFinanceRatioFromBands(bands = [], propertyValue) {
  ...
  if (!Number.isFinite(numericPropertyValue)) {
    return bands[0].financeRatio;
  }

  const matchedBand = bands.find(
    (band) =>
      numericPropertyValue >= (band.minPropertyValue ?? 0) &&
...
function calculateSuitableUnitValue(program, downPayment, maxAffordableLoan) {
  ...
  if (bestFit < program.minPropertyValue) {
    return 0;
  }
...
function evaluateProgram(data, program) {
  ...
  const maxInstallment = Math.max(
    0,
    Math.round((data.income || 0) * MAX_INSTALLMENT_RATIO - (data.obligations || 0))
  );
  const maxAffordableLoan = calculateLoanFromInstallment(
    maxInstallment,
    program.annualRate,
    program.maxYears
  );
  ...
  const reasons = [];
  const propertyInRange =
    Number.isFinite(data.propertyValue) &&
  ...
  if (!incomeInRange) {
    reasons.push(`الدخل المطلوب للمبادرة دي بحد أقصى ${formatMoney(incomeCap)} جنيه.`);
  }

  if (!propertyInRange) {
    reasons.push(`سعر الوحدة في المبادرة دي لازم يكون ${formatPropertyRange(program)}.`);
  }

  if (maxInstallment <= 0) {
    reasons.push('الالتزامات الحالية مستهلكة الحد الآمن من القسط الشهري.');
  }

  if ((data.downPayment || 0) < minimumDownPayment) {
    reasons.push(`المقدم المطلوب يبدأ من ${formatMoney(minimumDownPayment)} جنيه.`);
  }

  if (requestedLoan > maxAffordableLoan) {
    reasons.push('القسط المتوقع أعلى من 40% من صافي الدخل بعد خصم الالتزامات.');
  }
...
function pickBestEvaluation(data) {
  const evaluations = PROGRAMS.map((program) => evaluateProgram(data, program));
  const eligible = evaluations.filter((evaluation) => evaluation.isEligible);

  if (eligible.length > 0) {
    return eligible.sort((left, right) => left.program.annualRate - right.program.annualRate)[0];
  }
  ...
  return evaluations.sort((left, right) => {
    ...
    return left.program.annualRate - right.program.annualRate;
  })[0];
}
...
function buildQuestionResponse(data = {}, capturedData = {}, stage = '') {
  ...
  if (nextStep === 'collect_name') {
    return buildResponse(
      'ممكن أعرف اسم حضرتك؟',
      'collect_name',
      'اكتب اسم حضرتك عشان نكمل خطوة خطوة 👇'
    );
  }

  if (nextStep === 'collect_marital_status') {
    const greeting =
      capturedData.name && stage === 'collect_name'
  ...
  if (nextStep === 'collect_down_payment') {
    ...
    return buildResponse(
      `تمام.\nالحد الأقصى الآمن للقسط الجديد عندك تقريباً ${formatMoney(
        maxInstallment
      )} جنيه شهرياً.\nهل لديك مقدم؟ وإذا نعم، كم قيمته؟`,
      'collect_down_payment',
      'اكتب قيمة المقدم أو اكتب 0 لو لسه مفيش 👇'
    );
  }

  if (nextStep === 'collect_property_price') {
    return buildResponse(
      `المقدم المسجل عندك ${formatMoney(data.downPayment)} جنيه.\nما سعر الوحدة السكنية المتوقع؟`,
...
function buildValidationResponse(stage, data = {}) {
  ...
  if (stage === 'collect_down_payment') {
    return buildResponse(
      'هل لديك مقدم؟ وإذا نعم، كم قيمته؟ (اكتب الرقم أو 0)',
      'collect_down_payment',
      'اكتب رقم صحيح أكبر من أو يساوي صفر 👇'
    );
  }
...
function validateStepInput(stage, userMessage, capturedData, mergedData) {
  if (['collect_income', 'collect_obligations', 'collect_down_payment', 'collect_property_price'].includes(stage)) {
    if (hasNegativeNumber(userMessage)) {
      return buildValidationResponse(stage, mergedData);
    }
  }

  if (stage === 'collect_name' && !capturedData.name && !mergedData.name) {
    return buildValidationResponse(stage, mergedData);
  }

  if (stage === 'collect_marital_status' && !capturedData.maritalStatus && !mergedData.maritalStatus) {
    return buildValidationResponse(stage, mergedData);
  }

  if (stage === 'collect_income') {
    if (!Number.isFinite(capturedData.income) || capturedData.income <= 0) {
      return buildValidationResponse(stage, mergedData);
    }
  }

  if (stage === 'collect_obligations' && !Number.isFinite(capturedData.obligations)) {
    return buildValidationResponse(stage, mergedData);
  }

  if (stage === 'collect_down_payment' && !Number.isFinite(capturedData.downPayment)) {
    return buildValidationResponse(stage, mergedData);
  }

  if (stage === 'collect_property_price') {
    if (!Number.isFinite(capturedData.propertyValue) || capturedData.propertyValue <= 0) {
      return buildValidationResponse(stage, mergedData);
    }
  }
...
function buildAdvice(evaluation, data) {
  if (evaluation.isEligible) {
    if ((data.obligations || 0) > 0) {
      return 'وضعك المبدئي جيد، فقط حافظ إن الالتزامات الحالية ما تزيدش أثناء التقديم.';
    }

    if ((data.downPayment || 0) <= evaluation.requiredDownPayment + 50_000) {
      return 'أنت قريب جداً من أفضل سيناريو، وزيادة بسيطة في المقدم قد تديك راحة أكبر في القسط.';
    }

    return 'أنت مؤهل مبدئياً بشكل جيد، والأفضل تجهز مستندات الدخل والبطاقة قبل التقديم.';
  }

  if ((data.downPayment || 0) < evaluation.requiredDownPayment) {
    return `لو قدرت تزود المقدم إلى حوالي ${formatMoney(
      evaluation.requiredDownPayment
    )} جنيه هتبقى فرصتك أفضل.`;
  }

  if (evaluation.maxInstallment <= 0) {
    return 'الأفضل تقلل الالتزامات الحالية أو تختار وحدة بسعر أقل قبل التقديم.';
  }

  return `الأنسب ليك حاليًا وحدة في حدود ${formatMoney(
    evaluation.suitableUnitValue
  )} جنيه أو أقل.`;
}
...
function buildQualificationResponse(data = {}) {
  ...
  const affordabilityLine =
    ...
    data.propertyValue > evaluation.suitableUnitValue
      ? '- الوحدة الحالية أعلى من قدرتك المالية.'
      : null;
  const requestedUnitInstallment = calculateMonthlyInstallment(
    evaluation.requestedLoan,
    evaluation.program.annualRate,
    evaluation.program.maxYears
  );

  return buildResponse(
    [
      `أهلاً يا ${getFirstName(data.name)} 👋 خليني ألخصهالك ببساطة:`,
      statusLine,
      affordabilityLine,
      `- نوع المبادرة الأنسب ليك: ${evaluation.program.label}.`,
      `- أقصى قسط مناسب ليك بعد خصم الالتزامات: ${formatMoney(
        evaluation.maxInstallment
      )} جنيه شهرياً.`,
      evaluation.availableFinance > 0
        ? `- قيمة التمويل المتاحة تقريبياً: ${formatMoney(
            evaluation.availableFinance
          )} جنيه على مدة تصل إلى ${evaluation.program.maxYears} سنة.`
        : '- لا يوجد تمويل متاح حاليًا على نفس الوحدة بالبيانات الحالية.',
      `- المقدم المطلوب للوحدة المستهدفة: ${formatMoney(
        evaluation.requiredDownPayment
      )} جنيه.`,
      `- القسط المتوقع على وحدتك الحالية: ${formatMoney(
        requestedUnitInstallment
      )} جنيه شهرياً.`,
      evaluation.suitableUnitValue > 0
        ? `- أقصى سعر وحدة مناسب ليك حالياً: حوالي ${formatMoney(
            evaluation.suitableUnitValue
          )} جنيه.`
        : '- تحتاج مقدم أعلى أو التزامات أقل قبل ما نحدد سعر وحدة مناسب بشكل منطقي.',
      evaluation.adminFees > 0
        ? `- المصروفات الإدارية التقريبية: ${formatMoney(evaluation.adminFees)} جنيه.`
        : null,
...
function buildPhoneCapturedResponse(data = {}) {
  ...
  return buildResponse(
    [
      `تمام يا ${getFirstName(data.name)}، سجلنا رقمك ${data.phone}.`,
      `- المبادرة الأنسب ليك حاليًا: ${evaluation.program.label}.`,
      `- التمويل المتاح تقريبياً: ${formatMoney(evaluation.availableFinance)} جنيه.`,
      `- القسط المتوقع على السيناريو الحالي: ${formatMoney(
        evaluation.expectedInstallment
      )} جنيه شهرياً.`,
...
async function getChatResponse({
  ...
  if (nextStep !== 'collect_phone' && nextStep !== 'lead_saved') {
    return {
      ...buildQuestionResponse(mergedData, capturedData, stage),
      dataCaptured: capturedData
    };
  }
  ...
  if (nextStep === 'collect_phone') {
    return {
      ...buildQualificationResponse(mergedData),
      dataCaptured: capturedData
    };
  }

  return {
    ...buildPhoneCapturedResponse(mergedData),
    dataCaptured: capturedData
  };
}
